...
def emit_mono_bdf_header(bdf: BdfRecord, stream: TextIO):
    """Outputs a weegfx C header file storing a FONT to `stream`.
    Only accepts monospace fonts!"""
    if bdf.type != 'FONT':
    ...
    cw, ch, cox, coy = bbox = bdf.items['FONTBOUNDINGBOX']
    font_family = getval(properties, 'FAMILY_NAME', '<unknown family>')
    font_weight = getval(properties, 'WEIGHT_NAME', '')

    h_guard = f'{normname(font_family.upper())}_{normname(font_weight.upper())}_{cw}_{ch}_H'
...
//
// Font: {getval(properties, 'FAMILY_NAME', '<unknown family>')} {cw}x{ch} {getval(properties, 'WEIGHT_NAME', '')}
...
if __name__ == '__main__':
    with open('/tmp/test.bdf') as infile:
        ...
        emit_mono_bdf_header(bdf, sys.stdout)
